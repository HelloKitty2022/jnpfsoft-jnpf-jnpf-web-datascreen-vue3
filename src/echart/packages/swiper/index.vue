<template>
  <div :class="[b(),className]" :style="styleSizeName" ref="main">
    <el-carousel :style="styleChartName" :type="type" :indicator-position="indicator"
      ...
      <el-carousel-item v-for="(item,index) in dataChart" :key="index"
        ...
        <img v-if="typeList.img.test(item.value)" :src="computedImgUrl(item.value)"
          draggable="false" :style="styleName" />
        <video muted v-bind="params" v-else-if="typeList.video.test(item.value)"
          :src="computedImgUrl(item.value)" :style="styleName">
        </video>
        <avue-echart-clapper v-else-if="item.type=='hls'" :width="width" :height="height"
          :key="index" :data="{value:item.value}" :option="hlsOption"></avue-echart-clapper>
        <avue-echart-iframe v-else-if="item.type=='iframe'" :width="width" :height="height"
