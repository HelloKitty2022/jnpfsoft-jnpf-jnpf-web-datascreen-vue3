<template>
  <div class="imgTabs" :style="styleSizeName" :class="className">
    <div :style="styleChartName">
      <div class="imgTabs__list">
        <div class="imgTabs__item" :class="{'imgTabs--active':index==count}"
          v-for="(item,index) in dataChart" :key="index" @click="handleItem(index)">
          <span>{{item.text}}</span>
        </div>
      </div>
      <div class="imgTabs__carousel">
        <el-carousel :direction="option.direction" :interval="option.interval"
          :autoplay="option.autoplay">
          <el-carousel-item v-for="(item,index) in active" :key="index">
